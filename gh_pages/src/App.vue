<template>
  <div id="app">
    <Header ref="headerComponent" @toggleMenu="toggleMenu" @updateSearchKeywords="updateSearchKeywords"
      @updateSearchResults="updateSearchResults" @searchTopic="searchTopic" />
    <main>
      ...
      <div class="container" v-else>
        <h1 class="results__heading" v-if="searchResults.length && searchKeywords.includes('tag:')">
          {{ searchResults.length }} tags matching "{{ searchKeywords.replace('tag:', '') }}".</h1>
        <h1 class="results__heading" v-else-if="searchResults.length">{{ searchResults.length }} results matching
          "{{ searchKeywords }}".</h1>
        <h1 class="results__heading" v-else-if="!searchResults.length && searchKeywords.includes('tag:')">No tags matching
          "{{ searchKeywords.replace('tag:', '') }}".</h1>
        <h1 class="results__heading" v-else>No results matching "{{ searchKeywords }}".</h1>

        <ul class="results__search">
          <li class="result" v-for="(result, index) in searchResults" :key="index">
            <header class="result__header">
              <router-link :to="result.path">
                <h2 class="result__title">{{ result.name }}</h2>
              </router-link>

              <span tabindex="0" v-for="(tag, idx) in result.tags" :key="idx" class="tag" @keyup.enter="searchTopic(tag)"
